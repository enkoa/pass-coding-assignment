...
int main(int argc, char *argv[]) {
  ...
  if (argc > 3) {
    fprintf(stderr, "Too many comman arguments\n");
    return(-1);
  } else if(argc == 2) {
    storage = init_storage(argv[1]);
  } else {
    storage = init_storage("storage.bin");
  }

  if (storage == NULL) { // return -1 if file cannot be open
    fprintf(stderr, "Error in opening file from main\n");
    return(-1);
  }
...
void inputLoop(unsigned char buf[], int bufferSize, STORAGE *storage) {
  ...
  while (fgets(in_buffer, 100, stdin)) {
    ...
    *arg++ = strtok(in_buffer, SEPARATORS);
	...
	if (result < 0) {
	  fprintf(stderr, "Error in writing to put_bytes\n");
	}
      } else {
	...
	if (result == 0) {
	  ...
	  fprintf(stderr, "Error in reading from get_bytes\n");
	}
      } else {
